...
class Jetpack_Tiled_Gallery {
	...
	public function rectangular_talavera( $attachments ) {
		...
		foreach ( $grouper->grouped_images as $row ) {
          ...
          //vardump($image);
          //added credits - should optimize later - 8/21/13 - jhaun@handbrewed.com
          $credits = get_post_meta($image->ID,'_flo_credits',true);
          $size = 'large';
					if ( $image->width < 250 )
						$size = 'small';
          ...
          //removed the <a href="' . esc_url( $link ) . '">  and correstponding </a> tag below
          //reference ticket - http://plugins.trac.wordpress.org/ticket/1854#ticket - jhaun@handbrewed.com
          //added pinterest icon to all images in tiled gallery display - 8/21/13  - jhaun@handbrewed.com
          $img_md5 = md5(esc_url($img_src));
          $pinit = "javascript:void(showPinterest('".$img_md5."'))";
          $bottom_info = '<a href="'.$pinit.'" class="pinit"></a>';
					$output .= '<div class="tiled-gallery-item tiled-gallery-item-' . esc_attr( $size ) . ' image" id="'.$img_md5.'"><img ' . $this->generate_carousel_image_args( $image ) . ' src="' . esc_url( $img_src ) . '" width="' . esc_attr( $image->width ) . '" height="' . esc_attr( $image->height ) . '" align="left" title="' . esc_attr( $image_title ) . '" />'.$bottom_info;
          ...
          //modified to use new $credits instead of $image->post_excerpt below
          if ( trim( $credits) )
						$output .= '<div class="tiled-gallery-caption">Photography: ' . wptexturize( $credits ) . '</div>';
